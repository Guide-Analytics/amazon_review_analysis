...
def generateReport(productName, productURL):
    ...
    try:
        ...
        cur.execute('''
            SELECT DISTINCT  R.author_id, R.rating, P.average_rating, R.date, A.num_of_reviews, A.word_reptition_score,
            FROM review_table R
        ...
        while rating is not None:
            date = rating[3]
            numRev = rating[4]
            wordRepScore = rating[5]
            ratingData[rating[0]] = [int(rating[1]), float(rating[2]), date, numRev, wordRepScore]
            rating = cur.fetchone()
        ...
        cur.execute('''
            ...
            R.review_text, R.verified_purchase, R.review_score, R.date,  R.review_id, A.author_id, A.num_of_reviews,
            A.word_reptition_score
            FROM review_table R
            JOIN product_table P ON P.product_id = R.product_id
            JOIN author_table A ON A.author_id = R.author_id
        ''')
        ...
        cur.execute('''
           ...
           A.word_reptition_score, R.review_id, A.author_id
           FROM review_table R
           JOIN product_table P ON P.product_id = R.product_id
           JOIN author_table A ON A.author_id = R.author_id
        ''')
...
def authorInfoReport(authorInfoRow, authorData, lowQualityWords):
    ...
    authorData.append([author, numOfReviews, productURL, rating, averageRating,
                       ...
                       verifiedPurchase, wordPercentage, wordRScore])
...
def sentimentReport(sentimentRow, sentimentData, productURL, lowQualityWords, highQualityWords):
    ...
    if productURL == produrlscraped:
        ...
        numRev = sentimentRow[10]
        wordRScore = sentimentRow[11]

        sentimentData.append([rating, averageRating, author, reviewText, review_date,
                             verifiedPurc, highQualityWordCount, wordCount, lowQualityWordCount,
                             sentimentScore, numRev, wordRScore])
...
def keyWordReport(keywordRow, keyWordData, productURL, ratingData):
    ...
    for sentences in lstofsentences:
        ...
        if productURL == producturlscraped:
            ...
            # SentenceLabelling Information
            authorID = sentences[1]
            author = sentences[2]
            splitScores = sentences[3].split(', ')

            # Rating Data from Review/Author Database
            ...
            keyWordData.append([keywords, keywordsSentScore, keywordsConfScore,
                                text, sentimentScore, confidenceScore, author,
                                date, rating[0], rating[1], numRev, wordRScore, "NULL"])
...
def outputToCSV(keyWordData, sentimentData, authorData, productName, productURL, dateScraped):
    ...
    try:
        with open(productInfo, 'wb') as csvfile:
            ...
            sentwriter.writerow(["Product Info"])
            sentwriter.writerow(["KEYWORDS", "Keywords Sentiments Score", "Keywords Confidence Score", "Sentence",
                                 "Sentiment Score", "Confidence Score", "Author", "Review Date", "Rating", "Product Rating",
                                 "# Of Review", "Word Repetition Score", "Category"])
            sentwriter.writerows(keyWordData)
            sentwriter.writerow([" ", productname+" || "+datescraped, "", producturl])
    except:
    ...
    try:
        with open(sentimentInfo, 'wb') as csvfile:
            ...
            sentwriter = csv.writer(csvfile)
            sentwriter.writerow(["Sentiment Info"])
            sentwriter.writerow(["Rating", "Product Rating", "Author", "Review Text", "Review Date", "Verified Purchase",
                                 "# HQ Words", "Word Count", "# LQ Words", "Sentiment Score", "# Of Reviews", "Word Repetition Score"])
    ...
    with open(authorInfo, 'wb') as csvfile:
        ...
        sentwriter.writerow(["Author", "# Of Reviews", "Product URL", "Rating", "Product Rating", "Review Text", "Review Date",
