...
'''  
def productFound(char_sent, filenum):
    
    sFilename = 'sentenceLabellingProd'
    ext = '.csv'
        
    with open(sFilename+str(filenum)+ext,'wb') as csvfile:
        sentwriter = csv.writer(csvfile)
        sentwriter.writerow(['Characteristic', 'Sentence Containing Characteristics'])     
        sentwriter.writerows(char_sent.items())
'''